#!/usr/bin/env node
// Build FateCore v0.3-pub target-journal-aware training matrix.
//
// Starts from the v0.3 enriched CSV but keeps only:
//   - Round 3 pre-submission features
//   - target-journal optional prior-year j_hist_* features
//
// The j_hist_* columns in the source v0.3 matrix were generated with
// journal_year_metrics rows where metric_year < paper.year. Same-year journal
// labels remain y_* only.
...
const OUT_PATH = arg('out', join(FEATURES_DIR, 'v0.3-pub-features.csv'))
const MANIFEST_PATH = arg('manifest', join(FEATURES_DIR, 'v0.3-pub-features-manifest.json'))
const LIMIT = Number(arg('limit', '0'))

const idCols = ['doi', 'pmid']
const prepubFeatureCols = [
  'year',
  'title_word_count',
  'abstract_word_count',
...
const targetJournalFeatureCols = [
  'j_hist_metric_age',
  'j_hist_jcr_jif',
  'j_hist_jcr_jif_5yr',
  'j_hist_jci',
...
const featureCols = [...prepubFeatureCols, ...targetJournalFeatureCols]
...
async function main() {
  ...
  console.log('PaperFate FateCore v0.3-pub feature builder')
  console.log(`Input:    ${IN_PATH}`)
  console.log(`Output:   ${OUT_PATH}`)
  console.log(`Manifest: ${MANIFEST_PATH}`)
  console.log(`Features: ${featureCols.length} (${prepubFeatureCols.length} prepub + ${targetJournalFeatureCols.length} target-journal optional)`)
  ...
  let rowsWithAnyTargetJournalFeature = 0
  ...
  for await (const line of rl) {
    ...
    if (targetJournalFeatureCols.some(c => cells[header.indexOf(c)] !== '')) {
      rowsWithAnyTargetJournalFeature++
    }
  ...
  const manifest = {
    ...
    rows_with_any_target_journal_feature: rowsWithAnyTargetJournalFeature,
    columns: outCols,
    feature_cols: featureCols,
    prepub_feature_cols: prepubFeatureCols,
    target_journal_feature_cols: targetJournalFeatureCols,
    ...
    notes: {
      policy: 'Single NaN-safe target-journal-aware model. j_hist_* is optional at inference; absent target journal means these features stay NaN.',
      j_hist_rule: 'Source v0.3 builder used latest journal_year_metrics row with metric_year < paper.year. Same-year metrics are labels only.',
      training_mask: 'train-fatecore-v0.3-pub.py masks target_journal_feature_cols to NaN on a random fraction of train/cal rows.',
      forbidden_groups: [
  ...
  console.log(`Rows with any target-journal feature: ${rowsWithAnyTargetJournalFeature.toLocaleString()}`)
